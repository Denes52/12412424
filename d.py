# d.py — сразу использует прокси из proxies.txt, без предварительной проверки
import os
import ssl
import socks
import asyncio
...
# Таймауты увеличены (в ms в логах вы видите большие задержки)
CONNECT_TIMEOUT = 20.0       # timeout для client.connect()
SEND_CODE_TIMEOUT = 20.0     # timeout для client.send_code_request()
IS_AUTH_TIMEOUT = 6.0

MAX_SEND_PER_REQUEST = 25    # сколько прокси максимум попытаться использовать за одну команду
SEND_CONCURRENCY = 3         # сколько TelegramClient одновременно запускаем
DELAY_BETWEEN_TASKS = 0.25   # небольшая пауза между запуском попыток
# ======================
...
def parse_proxy_line(line: str):
    """Поддерживаем ip:port и опционально ip:port:lat"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Берём первые N прокси (или все, если меньше)
    ...
    async def try_send_via_proxy(ip, port):
        ...
        async with sem:
            try:
                ...
                # Подключаемся с таймаутом — если не получится, просто логируем и возвращаемся
                try:
                ...
                try:
                    # Проверяем авторизацию коротким таймаутом
                    try:
    ...
    # Сохраняем успешные прокси (если были)
    if ok_list:
