...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("Принял. Сразу начинаю попытки через указанные HTTP-прокси...")
        ...
        async with sem:
            try:
                ...
                try:
                    ...
                    print(f"[connect fail] {ip}:{port} {repr(e)}")
                    return
                ...
                if not is_auth:
                    try:
                        ...
                        print(f"[fail] send_code_request {ip}:{port}: {repr(e_inner)}")

                await client.disconnect()
            except Exception as e_outer:
                print(f"[telethon error] {ip}:{port}: {repr(e_outer)}")
    ...
    await msg.edit_text(f"Готово. Попыток: {sent}. Успешные прокси: {len(ok_list)}.")
